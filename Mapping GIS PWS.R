...
mapImage <- get_map(location = c(lon = -147.8, lat = 60.5), 
  color = "color",
  source = "google",
  maptype = "terrain",
  crop=FALSE,
...
ggmap(mapImage)+ 
 ...
 geom_point(aes(x= Lat, y=Long, label=ID), data = Recoveries, color="red", pch=17, cex=6, alpha =0.5)
?readShapeLines()
...
areas.shp = readShapeLines(file.choose())
areas.shp = readShapeSpatial(file.choose())
depth.shp = readShapeSpatial(file.choose())

plot(areas.shp,axes=TRUE,ylim=c(58,61),xlim=c(-149.0,-146))
plot(depth.shp,axes=TRUE,)
plot(areas.shp, add=T)
...
proj4string(areas.shp) = CRS("+proj=aea +lat_1=55 +lat_2=65 +lat_0=50 +lon_0=-154 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs")
proj4string(depth.shp) = CRS("+proj=aea +lat_1=55 +lat_2=65 +lat_0=50 +lon_0=-154 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs")
??ellps
...
stat.area= spTransform(areas.shp, CRS.new)
depth= spTransform(depth.shp, CRS.new)
...
data_name<-"stat.area"
assign(data_name,spTransform(areas.shp, CRS("+proj=longlat")))
data_name<-"depth"
assign(data_name,spTransform(depth.shp, CRS("+proj=longlat")))
...
ylim <- c(bBox[2,1] - ynudge, bBox[2,2] +ynudge)
ak.13
########Location map
points(ak.13, pch=19, cex=1, col="red")
plot(ak.13, axes=TRUE, xlim=xlim, ylim=ylim, pch=19, cex=.5, col="red")
text(ak.13,labels=Recoveries$ADU, pos=runif(30,1,4), col="blue", offset=0.5, cex=.7)
plot(stat.area,add=T)
plot(depth, add=T)
lines(ak.countries, col="black")
lines(stat.area, col="black")
